...
interface ContentDNAData {
  _id: string;
  responses: ContentDNAAnswer[];
  creatorExamples?: Array<{ url: string; platform: string; extractedTranscript: string }>;
  contentSamples?: Array<{ text: string; type: string }>;
  completedAt?: string;
}

interface PersonalBaselineAnswer {
  questionId: string;
  question: string;
  answer: string;
}

interface PersonalBaselineData {
  _id: string;
  responses: PersonalBaselineAnswer[];
  completedAt?: string;
}

// --- New 4-tab structure ---
type TabKey = "overview" | "content" | "contentDNA" | "studentProfile";

const TABS: { key: TabKey; label: string }[] = [
  { key: "overview", label: "Overview" },
  { key: "content", label: "Content" },
  { key: "contentDNA", label: "Content DNA" },
  { key: "studentProfile", label: "Student Profile" },
];

// --- Content sub-tabs ---
type ContentSubTab = "tasks" | "ideas" | "scripts" | "brainDumps" | "voiceStorming" | "aiDocuments";
...
export default function StudentDetailPage() {
  ...
  const [contentSubTab, setContentSubTab] = useState<ContentSubTab>("tasks");
  ...
  const [deleteTaskTarget, setDeleteTaskTarget] = useState<{ _id: string; title: string } | null>(null);
  const [isDeletingTask, setIsDeletingTask] = useState(false);

  // Delete AI document confirm state
  const [deleteDocTarget, setDeleteDocTarget] = useState<{ _id: string; title: string } | null>(null);
  ...
  // Content DNA state (new)
  const [contentDNA, setContentDNA] = useState<ContentDNAData | null>(null);
  const [contentDNALoading, setContentDNALoading] = useState(false);
  const [contentDNALoaded, setContentDNALoaded] = useState(false);

  // Personal Baseline state (new)
  const [personalBaseline, setPersonalBaseline] = useState<PersonalBaselineData | null>(null);
  const [personalBaselineLoading, setPersonalBaselineLoading] = useState(false);
  const [personalBaselineLoaded, setPersonalBaselineLoaded] = useState(false);
  ...
  // Lazy load: Ideas (when Content tab + Ideas sub-tab active)
  useEffect(() => {
    if (((activeTab === "content" && contentSubTab === "ideas") || activeTab === "overview") && !ideasLoaded) {
  ...
  }, [activeTab, contentSubTab, id, ideasLoaded]);

  useEffect(() => {
    if (((activeTab === "content" && contentSubTab === "scripts") || activeTab === "overview") && !scriptsLoaded) {
  ...
  }, [activeTab, contentSubTab, id, scriptsLoaded]);

  useEffect(() => {
    if (activeTab === "content" && contentSubTab === "brainDumps" && !brainDumpsLoaded) {
  ...
  }, [activeTab, contentSubTab, id, brainDumpsLoaded]);

  useEffect(() => {
    if (activeTab === "content" && contentSubTab === "voiceStorming" && !voiceStormsLoaded) {
  ...
  }, [activeTab, contentSubTab, id, voiceStormsLoaded]);

  // Brand Brain: load for Content DNA tab (content pillars, TOV, etc.)
  useEffect(() => {
    if ((activeTab === "contentDNA") && !brandBrainLoaded) {
  ...
  // Compiled profile: load for Student Profile tab
  useEffect(() => {
    if (activeTab === "studentProfile" && !compiledProfileLoaded) {
  ...
  useEffect(() => {
    if (activeTab === "content" && contentSubTab === "aiDocuments" && !aiDocsLoaded) {
  ...
  }, [activeTab, contentSubTab, id, aiDocsLoaded]);

  // Content DNA: lazy load for Content DNA tab
  useEffect(() => {
    if (activeTab === "contentDNA" && !contentDNALoaded) {
      setContentDNALoading(true);
      fetch(`/api/admin/content-dna?userId=${id}`)
        .then((res) => res.ok ? res.json() : null)
        .then((data) => {
          if (data?.data) setContentDNA(data.data);
          setContentDNALoaded(true);
        })
        .catch(() => {})
        .finally(() => setContentDNALoading(false));
    }
  }, [activeTab, id, contentDNALoaded]);

  // Personal Baseline: lazy load for Student Profile tab
  useEffect(() => {
    if (activeTab === "studentProfile" && !personalBaselineLoaded) {
      setPersonalBaselineLoading(true);
      fetch(`/api/admin/personal-baseline?userId=${id}`)
        .then((res) => res.ok ? res.json() : null)
        .then((data) => {
          if (data?.data) setPersonalBaseline(data.data);
          setPersonalBaselineLoaded(true);
        })
        .catch(() => {})
        .finally(() => setPersonalBaselineLoading(false));
    }
  }, [activeTab, id, personalBaselineLoaded]);

  // Also load brand brain for student profile tab (equipment checklist, ingest transcript)
  useEffect(() => {
    if (activeTab === "studentProfile" && !brandBrainLoaded) {
      setBrandBrainLoading(true);
      fetch(`/api/brand-brain?userId=${id}`)
        .then((res) => res.ok ? res.json() : null)
        .then((data) => {
  ...
  const handleDeleteTaskConfirm = async () => {
    ...
    try {
      const res = await fetch(`/api/tasks/${deleteTaskTarget._id}`, { method: "DELETE" });
      if (res.ok) {
  ...
  // Suppress unused warning for handleAddComment
  ...
  const taskCompletionPct = totalThisWeek > 0 ? Math.round((completedThisWeek / totalThisWeek) * 100) : 0;
  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        {activeTab === "overview" && (
          <div className="space-y-6">
            {/* Student info card (moved from above tabs) */}
            <div className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-card)] p-6 shadow-[var(--shadow-sm)]">
              ...
              {student && (
                <div className="mt-5 grid gap-x-8 gap-y-3 sm:grid-cols-3">
                  <div>
                    <p className="text-xs font-medium text-[var(--color-text-muted)]">Enrolled</p>
                    <p className="text-sm text-[var(--color-text-primary)]">
                      {student.createdAt
                        ? formatDate(student.createdAt, { month: "long", day: "numeric", year: "numeric" })
                        : "Unknown"}
                    </p>
                  </div>
                  <div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <div className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-card)] p-4 shadow-[var(--shadow-sm)]">
                <p className="text-xs font-medium text-[var(--color-text-muted)]">Ideas</p>
                <p className="mt-1 text-2xl font-semibold text-[var(--color-text-primary)]">
                  {ideasLoaded ? ideas.length : "-"}
                </p>
              </div>
              <div className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-card)] p-4 shadow-[var(--shadow-sm)]">
                <p className="text-xs font-medium text-[var(--color-text-muted)]">Scripts</p>
                <p className="mt-1 text-2xl font-semibold text-[var(--color-text-primary)]">
                  {scriptsLoaded ? scripts.length : "-"}
                </p>
              </div>
              <div className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-card)] p-4 shadow-[var(--shadow-sm)]">
                <p className="text-xs font-medium text-[var(--color-text-muted)]">Week {selectedWeek} Tasks</p>
                <p className="mt-1 text-2xl font-semibold text-[var(--color-text-primary)]">
                  {completedThisWeek}/{totalThisWeek}
                </p>
              </div>
              <div className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-card)] p-4 shadow-[var(--shadow-sm)]">
                <p className="text-xs font-medium text-[var(--color-text-muted)]">Completion</p>
                <p className="mt-1 text-2xl font-semibold text-[var(--color-text-primary)]">
                  {taskCompletionPct}%
                </p>
              </div>
            ...
            {student && <BodyDoubleAssignment studentId={student._id} />}
            ...
            <div className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-card)] p-5 shadow-[var(--shadow-sm)]">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between rounded-[var(--radius-md)] bg-[var(--color-bg-secondary)] px-3 py-2">
                  <span className="text-sm text-[var(--color-text-primary)]">Tasks this week</span>
                  <span className="text-sm font-medium text-[var(--color-text-primary)]">
                    {completedThisWeek} of {totalThisWeek} completed
                  </span>
                </div>
                <div className="flex items-center justify-between rounded-[var(--radius-md)] bg-[var(--color-bg-secondary)] px-3 py-2">
                  <span className="text-sm text-[var(--color-text-primary)]">Content ideas</span>
                  <span className="text-sm font-medium text-[var(--color-text-primary)]">
                    {ideasLoaded ? ideas.length : "..."}
                  </span>
                </div>
                <div className="flex items-center justify-between rounded-[var(--radius-md)] bg-[var(--color-bg-secondary)] px-3 py-2">
                  <span className="text-sm text-[var(--color-text-primary)]">Scripts</span>
                  <span className="text-sm font-medium text-[var(--color-text-primary)]">
                    {scriptsLoaded ? scripts.length : "..."}
                  </span>
                </div>
                <div className="flex items-center justify-between rounded-[var(--radius-md)] bg-[var(--color-bg-secondary)] px-3 py-2">
                  <span className="text-sm text-[var(--color-text-primary)]">Onboarding</span>
                  <span className="text-sm font-medium text-[var(--color-text-primary)]">
                    {student?.onboardingCompleted ? "Completed" : "In Progress"}
                  </span>
                </div>
              </div>
            </div>
        ...
        {activeTab === "content" && (
          <div className="space-y-6">
                            ...
                            return (
                              <div key={task._id} className="group relative">
                                ...
                                <div className="absolute right-3 top-3 flex gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                                  ...
                                  <button
                                    onClick={() => setDeleteTaskTarget({ _id: task._id, title: task.title })}
            ...
            {/* ---- Voice Storming Sub-tab ---- */}
            {contentSubTab === "voiceStorming" && (
              <div className="space-y-4">
            ...
            {/* ---- AI Documents Sub-tab ---- */}
            {contentSubTab === "aiDocuments" && (
                              ...
                              >
                                <div className="mb-2 flex items-start justify-between gap-3">
                                  ...
                                  <div className="flex shrink-0 gap-2">
                                    ...
                                    <button
                                      onClick={() => setDeleteDocTarget({ _id: doc._id, title: doc.title })}
        ...
        {/* ========== CONTENT DNA TAB ========== */}
        {activeTab === "contentDNA" && (
          <div className="space-y-6">
            {/* Content DNA Questionnaire Responses */}
            <div className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-card)] p-5 shadow-[var(--shadow-sm)]">
              <h3 className="mb-4 text-sm font-semibold text-[var(--color-text-primary)]">
                Content DNA Questionnaire
              </h3>
              {contentDNALoading && <LoadingSkeleton rows={4} />}
              {!contentDNALoading && !contentDNA && (
                <p className="text-sm text-[var(--color-text-muted)]">
                  This student has not completed the Content DNA questionnaire yet.
                </p>
              )}
              {!contentDNALoading && contentDNA && (
                <div className="space-y-4">
                  {contentDNA.completedAt && (
                    <p className="text-xs text-[var(--color-text-muted)]">
                      Completed: {formatDate(contentDNA.completedAt, { month: "long", day: "numeric", year: "numeric" })}
                    </p>
                  )}
                  {contentDNA.responses.map((r, i) => (
                    <div key={i} className="rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-bg-secondary)] p-3">
                      <p className="mb-1 text-xs font-medium text-[var(--color-text-muted)]">{r.question}</p>
            ...
            {tovGuide && (
              <div className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-card)] p-5 shadow-[var(--shadow-sm)]">
                ...
                <p className="mt-2 text-xs text-[var(--color-text-muted)]">
                  Use the review card on the Overview tab to edit or change the Tone of Voice status.
                </p>
              </div>
            )}
            ...
            {!brandBrainLoading && !brandBrain && (
              <EmptyState message="Brand Brain not set up yet. Content pillars will appear here once created." />
            )}

            {/* Written Samples */}
            {!contentDNALoading && contentDNA && contentDNA.contentSamples && contentDNA.contentSamples.length > 0 && (
              <div className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-card)] p-5 shadow-[var(--shadow-sm)]">
                <h3 className="mb-4 text-sm font-semibold text-[var(--color-text-primary)]">
                  Written Samples
                </h3>
                <div className="space-y-3">
                  {contentDNA.contentSamples.map((sample, i) => (
                    <div
                      key={i}
                      className="rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-bg-secondary)] p-3"
                    >
                      <span className="mb-1 inline-block rounded-full bg-[var(--color-bg-tertiary)] px-2 py-0.5 text-xs text-white">
                        {sample.type.replace(/_/g, " ")}
                      </span>
                      <p className="mt-1 text-sm leading-relaxed text-[var(--color-text-primary)] whitespace-pre-wrap">
                        {sample.text}
                      </p>
                    </div>
                  ))}
            ...
            {!contentDNALoading && contentDNA && contentDNA.creatorExamples && contentDNA.creatorExamples.length > 0 && (
              <div className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-card)] p-5 shadow-[var(--shadow-sm)]">
                <h3 className="mb-4 text-sm font-semibold text-[var(--color-text-primary)]">
                  Creator Inspirations
                </h3>
                <div className="space-y-2">
                  {contentDNA.creatorExamples.map((ex, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-3 rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-bg-secondary)] p-3"
                    >
                      <span className="shrink-0 rounded-full bg-[var(--color-bg-tertiary)] px-2 py-0.5 text-xs text-white">
                        {ex.platform}
                      </span>
                      <a
                        href={ex.url}
                      ...
                      {ex.extractedTranscript && (
                        <span className="shrink-0 text-xs text-emerald-500">transcript available</span>
                      )}
                    </div>
                  ))}
                </div>
        ...
        {activeTab === "studentProfile" && (
          <div className="space-y-6">
            ...
            {/* Equipment Checklist */}
            {brandBrainLoading && <LoadingSkeleton rows={2} />}
            {!brandBrainLoading && brandBrain && brandBrain._id && (
              <EquipmentChecklist
            ...
            {/* Equipment Profile */}
            {!brandBrainLoading && brandBrain && brandBrain.equipmentProfile && (brandBrain.equipmentProfile.camera || brandBrain.equipmentProfile.location || brandBrain.equipmentProfile.constraints) && (
              <div className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-card)] p-5 shadow-[var(--shadow-sm)]">
                <h3 className="mb-4 text-sm font-semibold text-[var(--color-text-primary)]">
                  Equipment Profile
                </h3>
            ...
            {/* Personal Baseline Responses */}
            <div className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-card)] p-5 shadow-[var(--shadow-sm)]">
              <h3 className="mb-4 text-sm font-semibold text-[var(--color-text-primary)]">
                Personal Baseline Responses
              </h3>
              {personalBaselineLoading && <LoadingSkeleton rows={4} />}
              {!personalBaselineLoading && !personalBaseline && (
                <p className="text-sm text-[var(--color-text-muted)]">
                  This student has not completed the Personal Baseline questionnaire yet.
                </p>
              )}
              {!personalBaselineLoading && personalBaseline && (
                <div className="space-y-4">
                  {personalBaseline.completedAt && (
                    <p className="text-xs text-[var(--color-text-muted)]">
                      Completed: {formatDate(personalBaseline.completedAt, { month: "long", day: "numeric", year: "numeric" })}
                    </p>
                  )}
                  {personalBaseline.responses.map((r, i) => (
                    <div key={i} className="rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-bg-secondary)] p-3">
                      <p className="mb-1 text-xs font-medium text-[var(--color-text-muted)]">{r.question}</p>
                      <p className="text-sm leading-relaxed text-[var(--color-text-primary)] whitespace-pre-wrap">
                        {r.answer}
            ...
            {/* AI-Generated Profile Summary (Compiled Profile) */}
            <div className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-card)] p-5 shadow-[var(--shadow-sm)]">
            ...
            {/* Industry Data */}
            {!brandBrainLoading && brandBrain && brandBrain.industryData && (brandBrain.industryData.field || (brandBrain.industryData.keywords && brandBrain.industryData.keywords.length > 0)) && (
              <div className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-card)] p-5 shadow-[var(--shadow-sm)]">
                <h3 className="mb-4 text-sm font-semibold text-[var(--color-text-primary)]">
                  Industry Data
                </h3>
      ...
      {/* Delete AI document confirmation */}
      {deleteDocTarget && (
        <ConfirmDeleteModal
          itemType="document"
          ...
          onConfirm={async () => {
            ...
            try {
              const res = await fetch(`/api/admin/ai-documents/${deleteDocTarget._id}`, { method: "DELETE" });
              if (res.ok) {
                setAiDocuments((prev) => prev.filter((d) => d._id !== deleteDocTarget._id));
              }
            } catch (err) {
